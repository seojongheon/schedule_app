...
async function postHandler(request: Request, context: { params: Promise<{ inquiryId: string }> }) {
  ...
  try {
    ...
    const result = await repository().claim({ inquiryId, actorUserId: actor.userId, serviceRoles: actor.serviceRoles, requestId });
    return NextResponse.json({ ...result, requestId });
  } catch (error) {
    const status = error instanceof Error && error.message === 'Authentication required.' ? 401 : 403;
    return NextResponse.json({ error: { code: status === 401 ? 'authentication_required' : 'inquiry_claim_denied', message: '문의를 배정할 수 없습니다.', requestId } }, { status });
  }
}
