...
const SCHEDULE_COLUMNS = 'id,room_id,title,start_at,end_at,address,customer_phone,estimated_price,additional_info,status,owner_member_id,owner_name_snapshot,created_by_member_id,created_by_name_snapshot,created_at,updated_at';
...
type ScheduleRow = {
  ...
  created_by_member_id: string | null; created_by_name_snapshot: string; created_at: string; updated_at: string;
};
...
export async function getScheduleWorkspaceData(
  ...
  try {
    ...
    const schedules = scheduleRows.map<Schedule>((schedule) => ({
      ...
      createdByMemberId: schedule.created_by_member_id, createdByName: schedule.created_by_name_snapshot,
